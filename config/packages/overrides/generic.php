...
if ($dfsNfsPath = $_ENV['DFS_NFS_PATH'] ?? false) {
    ...
    if ($value = $_ENV['DFS_DATABASE_DRIVER'] ?? false) {
    ...
    if ($value = $_ENV['DFS_DATABASE_HOST'] ?? false) {
    ...
    if ($value = $_ENV['DFS_DATABASE_PORT'] ?? false) {
    ...
    if ($value = $_ENV['DFS_DATABASE_NAME'] ?? false) {
    ...
    if ($value = $_ENV['DFS_DATABASE_USER'] ?? false) {
    ...
    if ($value = $_ENV['DFS_DATABASE_PASSWORD'] ?? false) {
...
if (($pool = $_ENV['CACHE_POOL'] ?? false) && file_exists(dirname(__DIR__)."/cache_pool/${pool}.yaml")) {
...
if ($purgeType = $_ENV['HTTPCACHE_PURGE_TYPE'] ?? false) {
    $container->setParameter('purge_type', $purgeType);
}

if ($value = $_ENV['MAILER_TRANSPORT'] ?? false) {
    $container->setParameter('mailer_transport', $value);
}

if ($value = $_ENV['LOG_TYPE'] ?? false) {
    $container->setParameter('log_type', $value);
}

if ($value = $_ENV['SESSION_HANDLER_ID'] ?? false) {
    $container->setParameter('ezplatform.session.handler_id', $value);
}

if ($value = $_ENV['SESSION_SAVE_PATH'] ?? false) {
